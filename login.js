$(document).ready(function(){
    ...
    //email-id for sign-up
     //pass for sign-up
    var password = $(".password-signup").val();         var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])(?=.{8,})");
    // At least one lowercase letter, one uppercase letter, one number, one special character, and minimum length of 8 characters
    if(!strongRegex.test(password)){
        $(".password-signup-hide").text("Password must contain at least 8 characters including uppercase, lowercase, numbers, and special characters.");
    if(password=="")
